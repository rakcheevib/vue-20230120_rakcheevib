...
const { default: UiCalendarView } = require(global.getSolutionPath('./components/UiCalendarView'));
const { default: InternationalHolidaysCalendar } = require(global.getSolutionPath(
  './components/InternationalHolidaysCalendar',
));
...
const SELECTOR_CELL_CONTENT = '.calendar-view__cell-content';
...
describe('slots/UiCalendarView', () => {
  describe('MeetupsCalendar', () => {
    it('MeetupsCalendar должен использовать компонент UiCalendarView', () => {
      const wrapper = mountMeetupsCalendar();
      expect(wrapper.findComponent(UiCalendarView).exists()).toBeTruthy();
    });
  ...
  describe('InternationalHolidaysCalendar', () => {
    it('InternationalHolidaysCalendar должен использовать компонент UiCalendarView', () => {
      const wrapper = mount(InternationalHolidaysCalendar);
      expect(wrapper.findComponent(UiCalendarView).exists()).toBeTruthy();
    });

    it(`InternationalHolidaysCalendar должен вывести "International Programmers' Day" 7-го Января 2023 `, () => {
      advanceTo(new Date('2023-01-15'));
      const wrapper = mount(InternationalHolidaysCalendar);
      const events = wrapper
        .findAll(SELECTOR_CELL)[12]
        .findAll(SELECTOR_CELL_CONTENT)
        .map((w) => w.text());
      expect(events).toEqual(["International Programmers' Day"]);
    });
  });
});
